...
case "$suite" in
  all) ;;
  smoke) selection+=(-only-testing:CutlineUITests/EditorUITests/testCaptionTextSearchEditAndDelete -only-testing:CutlineUITests/EditorUITests/testTrimAndMutePersist -test-iterations 3) ;;
