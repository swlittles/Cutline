...
The development implementation separates Mobile Short (9:16) from YouTube Video (16:9). Short exports require confirmed per-source camera/gameplay crops and an editable hook. The native preview/export compositor renders 30% facecam, a permanent 2% black locally configured branding strip, and 68% gameplay. Hooks appear for the first three seconds of each timeline clip. Format-3 project files protect this behavior from being silently discarded by older apps.
...
A local real-recording review exercised twelve saved selections without rescanning or paid AI. Outputs passed full decoding and metadata checks for H.264 1080×1920 at 30 fps, 48 kHz AAC, source-range duration and single-mix audio. Source crops and editorial notes are private review artifacts, not shared application defaults. These remain review drafts; speech captions, loudness normalization and automatic camera detection were not part of that pass.

## Local creator settings and privacy

Fresh installs have no creator branding or player aliases. Creator text and optional platform mark persist only in the app's local support directory, with owner-only permissions and separate Dev/release profiles. Core export and batch export require explicit branding for mobile shorts; preview cache keys include the profile. Projects do not serialize the creator profile. Settings changes rebuild the preview without adding project edits or undo entries.
