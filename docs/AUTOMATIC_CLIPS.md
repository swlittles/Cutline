...
Review each candidate with **Preview** and **Why this clip?**. **Add to timeline** appends the source range and switches the timeline to Mobile Short as one undoable edit. **Export selected clips** makes a new folder containing independent MP4s, `.cutline` projects, and `analysis.json` with the scan settings, measurements, source timestamps and selection evidence. Exports use the explicitly selected **Export audio** track (Track 1 by default), independently of the analysis track. This avoids doubling the OBS audience mix with isolated microphone/game tracks. For a custom multitrack mix, append the clip and adjust source-track gains in the timeline. Exports use the current resolution/frame-rate settings and render the same stacked mobile layout shown in candidate preview. Timeline effects, captions and added layers are not copied. Editable projects reference the original recording, so retain that file. Cancellation or failure removes the unfinished batch; existing files are never overwritten.
