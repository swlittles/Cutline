...
Coordinates use the displayed frame's top-left, after its rotation transform, and are fractions of width/height. The three starting rectangles are not accuracy-certified. Every OCR scan requires explicit calibration confirmation. For unsupported games, the initial rectangle is only an editable placeholder. Tarkov may have no usable elimination feed; use audio/motion/markers instead of claiming kill detection. Profile changes retain per-game edits, saved locally when a scan starts. Dev and production profiles use separate application-support directories.
